...
import {
  Plus, Trash2, Settings, LayoutDashboard, Keyboard, Printer, ChevronLeft, ChevronRight,
...
// Whole app state lives as ONE JSON document in the bin: { sheets: [...], months: { "2026-07": {...} } }
...
export default function App() {
  const [db, setDb] = useState(null); // { sheets: [...], months: { "yyyy-mm": {...} } }
  const [sheetId, setSheetId] = useState(null);
  const [date, setDate] = useState(todayISO());
  const [view, setView] = useState("input"); // input | dashboard | settings
  ...
  /* init: pull the single JSON document from JSONBin */
  useEffect(() => {
    (async () => {
      try {
...
function InputView({ sheet, date, setDate, monthData, updateEntry, clearEntry }) {
  ...
  // days already filled this month for this sheet
...
function SummaryCard({ label, value, color }) {
  return (
    <div style={{ flex: 1, background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: "10px 14px" }}>
      <div style={{ fontSize: 10.5, color: C.muted, textTransform: "uppercase", letterSpacing: 0.6 }}>{label}</div>
      <div className="num-field" style={{ fontSize: 20, fontWeight: 600, color: color || C.text }}>{value}</div>
    </div>
  );
}

function MetricCard({ sheetId, date, metric, updateEntry }) {
  const color = statusColor(metric.pct);
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderLeft: `4px solid ${color}`, borderRadius: 12, padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ fontWeight: 600, fontSize: 15 }}>{metric.name}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 11, color: C.muted, fontFamily: "'IBM Plex Mono', monospace" }}>CT {metric.ct}s · STD {metric.qs || 0}</span>
          <span className="num-field" style={{ fontSize: 22, fontWeight: 700, color, minWidth: 62, textAlign: "right" }}>
            {metric.pct === null ? "—" : `${metric.pct.toFixed(0)}%`}
          </span>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <NumField label="PCS (hasil)" value={metric.pcs} onChange={(v) => updateEntry(sheetId, date, metric.id, "pcs", v)} />
        <NumField label="MENIT (waktu)" value={metric.menit} onChange={(v) => updateEntry(sheetId, date, metric.id, "menit", v)} />
      </div>
    </div>
  );
}
...
function NumField({ label, value, onChange }) {
  return (
    <label style={{ display: "block" }}>
      <div style={{ fontSize: 10.5, color: C.muted, marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }}>{label}</div>
      <input
        type="text"
        inputMode="numeric"
        pattern="[0-9]*"
        ...
        style={{
          ...
          padding: "10px 12px", color: C.text, fontSize: 17, fontWeight: 600, outline: "none",
        }}
...
function DashboardView({ sheets, sheetId, setSheetId, mk, setDate, monthData }) {
  ...
  const chartData = dailyRows.map((r) => ({ name: String(r.day), pct: r.pct === null ? 0 : Math.round(r.pct) }));
  ...
  return (
    <div className="print-area" style={{ padding: 20, maxWidth: 960, margin: "0 auto" }}>
      ...
      <div className="print-card" style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: "16px 10px", marginBottom: 22, height: 220 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 4, right: 10, left: -20, bottom: 0 }}>
            ...
            <Tooltip contentStyle={{ background: C.panel2, border: `1px solid ${C.line}`, borderRadius: 8, fontSize: 12 }} labelFormatter={(l) => `Tanggal ${l}`} formatter={(v) => [`${v}%`, "%ACT"]} />
...
const th = { padding: "9px 14px", fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: 0.5, fontWeight: 600 };
...
/* -------------------------------- settings view -------------------------------- */
function SettingsView({ sheets, addSheet, removeSheet, updateSheetName, addMetric, updateMetric, removeMetric }) {
  const [newName, setNewName] = useState("");
  return (
    <div style={{ padding: 20, maxWidth: 720, margin: "0 auto" }}>
      <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 22, marginBottom: 4 }}>Kelola Line</div>
      <div style={{ color: C.muted, fontSize: 13, marginBottom: 18 }}>Tambah, ubah, atau hapus line produksi dan standar cycle time (CT) tiap metrik.</div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <input
          ...
          style={{ flex: 1, background: C.panel, border: `1px solid ${C.line}`, borderRadius: 8, padding: "10px 12px", color: C.text, fontSize: 13.5 }}
        />
        <button
          onClick={() => { if (newName.trim()) { addSheet(newName.trim()); setNewName(""); } }}
          style={{ background: C.amber, color: "#1A1D20", border: "none", borderRadius: 8, padding: "0 16px", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}
        >
          <Plus size={15} /> Tambah
        </button>
      </div>
          ...
          <div key={s.id} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: 14 }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
              <input
                ...
                style={{ flex: 1, background: C.panel2, border: `1px solid ${C.line}`, borderRadius: 8, padding: "8px 10px", color: C.text, fontWeight: 600, fontSize: 14 }}
              />
              <button
                onClick={() => { if (confirm(`Hapus line "${s.name}"? Data historis line ini tidak akan terhapus tapi tidak akan tampil lagi.`)) removeSheet(s.id); }}
                style={{ background: "transparent", border: `1px solid ${C.line}`, borderRadius: 8, padding: "0 10px", color: C.bad, cursor: "pointer" }}
              >
                <Trash2 size={15} />
              </button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {s.metrics.map((m) => (
                <div key={m.id} style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <input
                    ...
                    style={{ flex: 1, background: C.panel2, border: `1px solid ${C.line}`, borderRadius: 6, padding: "6px 8px", color: C.text, fontSize: 12.5 }}
                  />
                  <input
                    type="text" inputMode="decimal"
                    value={m.ct}
                    onChange={(e) => updateMetric(s.id, m.id, "ct", e.target.value.replace(/[^0-9.]/g, ""))}
                    placeholder="CT (detik)"
                    className="num-field"
                    style={{ width: 90, background: C.panel2, border: `1px solid ${C.line}`, borderRadius: 6, padding: "6px 8px", color: C.text, fontSize: 12.5 }}
                  />
                  <span style={{ fontSize: 11, color: C.muted }}>detik</span>
                  <button onClick={() => removeMetric(s.id, m.id)} style={{ background: "transparent", border: "none", color: C.muted, cursor: "pointer" }}>
                    <X size={14} />
                  </button>
                </div>
              ))}
              <button
                ...
                style={{ alignSelf: "flex-start", marginTop: 4, background: "transparent", border: `1px dashed ${C.line}`, borderRadius: 6, padding: "5px 10px", color: C.steel, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}
              >
